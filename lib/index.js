...
async function createIAFile(originalFilePath) {
    try {
        ...
        coc_nvim_1.window.showInformationMessage(`Fichier IA créé: ${iaFilePath}`);
    }
    catch (error) {
...
async function activate(context) {
    try {
        ...
        files
            ...
            .forEach(file => {
            coc_nvim_1.window.showInformationMessage(`Fichier trouvé: ${coc_nvim_1.workspace.asRelativePath(file.fsPath)}`);
        });
